...
import { useFigmaSettings } from '../hooks/useIntegrationData';
import { useUpdateFigmaSettings } from '../hooks/useIntegrationMutations';
import type { FigmaSettings, IntegrationItem } from '../types';

interface FigmaSettingsPanelProps {
  open: boolean;
  onClose: () => void;
  integration: IntegrationItem | null;
}

const DEFAULT_SETTINGS: FigmaSettings = {
  showThumbnails: true,
...
const SettingToggle: React.FC<{
  ...
  <label className="flex cursor-pointer items-center justify-between gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-gray-50 dark:hover:bg-white/5">
    ...
    <button
      ...
      className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#A259FF]/50 disabled:cursor-not-allowed disabled:opacity-50 ${checked ? 'bg-[#A259FF]' : 'bg-gray-300 dark:bg-gray-600'}`}
    >
...
export const FigmaSettingsPanel: React.FC<FigmaSettingsPanelProps> = ({ open, onClose, integration }) => {
  const [settings, setSettings] = useState<FigmaSettings>(DEFAULT_SETTINGS);
  const [initialized, setInitialized] = useState(false);
  const settingsQuery = useFigmaSettings({ enabled: open });
  const updateSettings = useUpdateFigmaSettings();
  ...
  const handleToggle = useCallback((key: keyof FigmaSettings, value: boolean) => {
    setSettings((prev) => ({ ...prev, [key]: value }));
    updateSettings.mutate({ [key]: value });
  }, [updateSettings]);

  const isSaving = updateSettings.isPending;
  const figmaUser = settingsQuery.data?.figmaUser;
        ...
        <>
          ...
          >
            <header className="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-border-dark">
              <h2 className="text-lg font-semibold">Figma Settings</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              <section className="space-y-1">
                ...
                <div className="space-y-0.5">
                  <SettingToggle label="Show design thumbnails on issues" checked={settings.showThumbnails} disabled={isSaving} onChange={(v) => handleToggle('showThumbnails', v)} />
                  <SettingToggle label="Show last modified date" checked={settings.showLastModified} disabled={isSaving} onChange={(v) => handleToggle('showLastModified', v)} />
                </div>
              </section>

              {figmaUser && (
                <section className="space-y-2 border-t border-gray-100 pt-6 dark:border-border-dark">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400">Figma Account</h3>
                  <div className="rounded-xl bg-gray-50 p-4 text-sm text-gray-500 dark:bg-white/5 dark:text-gray-400 space-y-1">
                    <p className="font-medium text-gray-700 dark:text-gray-200">@{figmaUser.handle}</p>
                    <p className="text-xs text-gray-400">{figmaUser.email}</p>
                  </div>
                </section>
              )}
