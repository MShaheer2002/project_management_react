...
import { MOCK_PROJECTS, MOCK_USERS, STATUS_LABELS, MOCK_DEPARTMENTS, MOCK_TEAMS } from '../constants';
import { CreatedTask, Priority, Status } from '../types';
import { motion, AnimatePresence } from 'motion/react';
import { RichTextEditor } from '../components/RichTextEditor';
import { generateNextTaskId, saveCreatedTask } from '../lib/task-storage';
...
export const CreateTaskPage: React.FC = () => {
  ...
  const handleCreate = () => {
    ...
    setTimeout(() => {
      ...
      saveCreatedTask(createdTask);
      setIsSubmitting(false);
      showToast(
        cleanSubtasks.length > 0
          ? `Task created with ${cleanSubtasks.length} subtask${cleanSubtasks.length > 1 ? 's' : ''}`
  ...
  }, [title, description, projectId, priority, status, assigneeId, departmentId, dueDate, dueTime, estimate, selectedLabels, subtasks]);
  ...
  return (
    ...
    >
      {/* Precision Header as per UI Guide Standard */}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide bg-white dark:bg-bg-dark">
          <div className="max-w-4xl mx-auto px-10 py-12 space-y-12 pb-32">
            ...
            <div className="space-y-6">
              <label className="text-[11px] font-bold uppercase tracking-[0.15em] text-gray-400 dark:text-gray-500 block px-1">Task Specification</label>
              <RichTextEditor 
                value={description}
                onChange={setDescription}
                placeholder="Describe this task's core requirements"
                minHeight="400px"
              />
            </div>

            {/* Scientific Subtasks System */}
            <div className="space-y-6 pt-12 border-t border-gray-100 dark:border-border-dark">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl border border-dashed border-gray-100 dark:border-border-dark focus-within:border-primary focus-within:bg-primary/5 focus-within:shadow-xl focus-within:shadow-primary/5 transition-all group">
                  ...
                  <input 
                    type="text" 
                    placeholder="Append a new requirement (Enter)..." 
                    value={newSubtask}
                      ...
                      {showLabelDropdown && (
                        ...
                        >
                          ...
                          <div className="pt-3 border-t border-gray-100 dark:border-border-dark">
                            <button className="w-full py-3 text-[10px] font-bold uppercase tracking-wide text-primary hover:bg-primary/5 rounded-xl transition-all border border-transparent hover:border-primary/20 flex items-center justify-center gap-2">
                              <Plus size={14} strokeWidth={3} /> New Classification
                            </button>
                          </div>
                        </motion.div>
                      )}
